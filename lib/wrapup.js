...
var handlePromise = function(promise, wrup, callback){
    promise(wrup).then(function(code){
        if (code != null) wrup.emit("data", code)
        return [null, code]
    }, function(err){
        wrup.emit(wrup.watching ? "warn" : "error", err)
        return [err]
    }).then(function(params){
        if (!wrup.watching) wrup.emit("end")
        if (callback) callback.apply(wrup, params)
    })
}
...
var WrapUp = prime({
    ...
    browser: function(callback){
        ...
        handlePromise(require('./output/browser'), this, callback)
        return this
    },
    ...
    amd: function(callback){
        var err = this.go()
        if (err) return handleGoError(this, err)
        handlePromise(require('./output/amd'), this, callback)
        return this
    },
    ...
    graph: function(callback){
        var err = this.go()
        if (err) return handleGoError(this, err)
        handlePromise(require('./output/graph'), this, callback)
        return this
    },
